...
where $\kappa$ is a positive constant \cite{Kynclova:2015}. The choice of $\kappa$ is not relevant, as the relative information in the compositional parts stays the same.  A D-dimensional vector $\textbf{x} = (x_1,\ldots,x_D)^T$ is said to be compositional if it is part of $\mathbbm{S}^D$. Next we can induce a $(D-1)$-dimensional vector space on $\mathbbm{S}^D$ by perturbation and power transformation. For compositions $\textbf{x},\textbf{y} \in \mathbbm{S}^D$ and $\alpha \in \mathbb{R}$ they are defined respectively as \cite{Kynclova:2015}

\begin{equation*}
\textbf{x}\oplus \textbf{y} := \ClOp(x_1y_1,x_2y_2,\ldots,x_Dy_D)^T, \hspace{0.5cm} \alpha \odot \textbf{x} := \ClOp(x_1^\alpha,x_2^\alpha,\ldots,x_D^\alpha)^T.
...
Using $y^{-1} := \ClOp(y_1^{-1},y_2^{-1},\ldots,y_D^{-1})$, the inverse perturbation can be defined as 

\begin{equation*}
\textbf{x} \ominus \textbf{y} := \textbf{x} \oplus \textbf{y}^{-1}
\label{eq: Inverse Perturbation}
\end{equation*}

Now we further define an inner product in order to have an inner product space over the simplex $\mathbbm{S}^D$. For two compositions $\textbf{x},\textbf{y} \in \mathbbm{S}^D$ define the Aitchison inner product as 

\begin{equation*}
\left\langle \textbf{x},\textbf{y} \right\rangle_a := \frac{1}{2D}\sum_{i=1}^{D}\sum_{j=1}^{D}\log(\frac{x_i}{x_j})\log(\frac{y_i}{y_j}).
...
\begin{equation*}
\norm{\textbf{x}}_a^2 := \left\langle  \textbf{x},\textbf{x} \right\rangle_a, \hspace{0.5cm} d_a(\textbf{x},\textbf{y}) := \norm{\textbf{x} \ominus \textbf{y}}_a .
...
Let $\textbf{x}, \textbf{y} \in \mathbbm{S}^D$ be D-part compositions.
...
\begin{equation*}
\textbf{y}^{(k)} = alr_k(\textbf{x}) := \left(\log\left(\frac{x_1}{x_k}\right), \ldots, \log\left(\frac{x_{k-1}}{x_k}\right),\log\left(\frac{x_{k+1}}{x_k}\right),\ldots,\log\left(\frac{x_D}{x_k}\right)\right).
...
\begin{equation*}
\textbf{z} = (z_1,\ldots, z_D)^T = clr(\textbf{x}) := \left(\log\left(\frac{x_1}{g(\textbf{x})}\right),\ldots, \log\left(\frac{x_D}{g(\textbf{x})}\right)\right)^T .
\label{eq:clr Coefficients}
\end{equation*}

This transformation maps $\textbf{x}$ into the hyperplane $V = \left\{\textbf{z} \in \mathbb{R}^D: \sum_{i=1}^D z_i=0\right\} \subset \mathbb{R}^D$. Hence the transformed data is constrained, which is emphasised by the term 'coefficient' instead of 'coordinates' \cite{Filzmoser:2020}. It can be shown that the clr transformation is an isometry\cite{Egozcue:2003}. Therefore it holds 

\begin{gather*}
\left\langle  \textbf{x},\textbf{y} \right\rangle_a = \left\langle  clr(\textbf{x}),clr(\textbf{y}) \right\rangle_a, \\
d(\textbf{x},\textbf{y})_a = d(clr(\textbf{x}),clr(\textbf{y})) .
...
The isometric log-ratio (ilr) are closely related to the clr Coefficients. Assume the inverse clr transformation is isometric. Let $\left\{v_1,\ldots,v_n\right\}$ be an orthonormal base in in the D-dimensional hyperplane $V$. Then $\textbf{e}_i = clr^{-1}(v_i), i=1,\ldots,D-1$ is an orthonormal basis of the simplex $\mathbbm{S}$. For $\textbf{x} \in \mathbbm{S}$, the ilr transformation can then be defined as  
...
In addition to being isometric, the ilr transformation is also isomorph. Let $\textbf{x}, \textbf{y}$ be two compositions and $\alpha, \beta  \in \mathbbm{R}$. Then,

\begin{equation*}
ilr(\alpha \odot \textbf{x} \oplus \beta \odot \textbf{y}) = \alpha \cdot ilr(\textbf{x}) + \beta \cdot ilr(\textbf{y})
...
\begin{gather*}
\left\langle  \textbf{x},\textbf{y} \right\rangle_a = \left\langle  ilr(\textbf{x}), ilr(\textbf{y}) \right\rangle_a, \\
d(\textbf{x},\textbf{y})_a = d(ilr(\textbf{x}),ilr(\textbf{y})),\\
\norm{x}_a = \norm{ilr(x)} = \norm{u} .
...
\begin{gather*}
ilr(\textbf{x}) = (z_1,\ldots,z_{D-1})^T, \\
z_j = \sqrt{\frac{D-j}{D-j+1}}\log\left(\frac{x_j}{\sqrt[D-j]{\prod_{l=j+1}^D x_l}}\right), \hspace{0.2cm} j=1,\ldots, D-1 .
...
x_i = \exp\left( \sum_{j=1}^{i-1}\frac{1}{\sqrt{(D-j+1)(D-j)}}z_j + \sqrt{\frac{D-i}{D-i+1}z_i} \right), \hspace{0.2cm} i=2,\ldots, D-1, \\
...
Since we have established the basic setting we can now introduce compositional time series (CTS). A CTS $\left\{\bm{x}_t:t=1,\ldots,n \right\}$ can be defined as a series where $\bm{x}_t = \left(x_{1t},\ldots,x_{Dt}\right)^T \in \mathbbm{S}^D$. They are thus characterised by their positive components which sum up to a constant $\kappa$ for each point in time $t=1,\ldots,n$ 

\begin{equation*}
\sum_{i=1}^D x_{it} = \kappa, \hspace{0.2cm} x_i > 0, i=1,\ldots D \hspace{0.1cm}; t=1,\ldots, n. 
\label{eq:CTS characterisation}
\end{equation*} 
Let $\textbf{x}_t$ be a D-dimensional compositional vector measured at time $t$, $t = 1,\ldots,n$ and $\textbf{z}_t = ilr(x_t)$ be its ilr transformation determined by the matrix $\textbf{V}$. Then the VAR model with lag order $p$ is given by \cite{Kynclova:2015}

\begin{equation}
\textbf{z}_t = \textbf{c}_{\textbf{V}} + \textbf{A}_{\textbf{V}}^{(1)}\textbf{z}_{t-1} + \textbf{A}_{\textbf{V}}^{(2)}\textbf{z}_{t-2} + \ldots + \textbf{A}_{\textbf{V}}^{(p)}\textbf{z}_{t-p} + \bm{\epsilon}_{t}.
\label{eq:VAR model}
\end{equation}

where $\textbf{c}_{\textbf{V}} \in \mathbb{R}^{D-1}$ is a real vector, $\textbf{A}_{\textbf{V}}^{(i)} \in \mathbb{R}^{(D-1) \times (D-1)}$ are parameter matrices and $\bm{\epsilon}_t$ is a white noise process with covariance matrix $\bm{\Sigma_\epsilon}$. The observation $\textbf{z}_t$ therefore depends on the p past observations $\textbf{z}_{t-1},\ldots,\textbf{z}_{t-p}$. It can be shown, that two VAR(p) models resulting from different ilr transformations are compositionally equivalent which means that the same predictions are obtained \cite{Kynclova:2015}. 
