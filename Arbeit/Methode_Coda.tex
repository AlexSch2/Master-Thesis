...
Let $\left\{\bm{Y}_t:t\in \mathbb{N}, \bm{Y}_t \in \mathbb{N}_0^K \right\}_f$ be our time series for fridge $f$ and assume that $\bm{Y}_t$ is a D-dimensional compositional vector measured at time $t, t=1,\ldots,n$. Further, let $\textbf{u}_t = ilr(\bm{Y}_t)$ be its ilr transformation determined by the matrix $\textbf{V}$. Then the VAR model with lag order $p$ is given by \cite{Kynclova:2015}
...
where $\textbf{c}_{\textbf{V}} \in \mathbb{R}^{D-1}$ is a real vector, $\textbf{A}_{\textbf{V}}^{(i)} \in \mathbb{R}^{(D-1) \times (D-1)}$ are parameter matrices and $\bm{\epsilon}_t$ is a white noise process with covariance matrix $\bm{\Sigma_\epsilon}$. The observation $\textbf{u}_t$ therefore depends on the p past observations $\textbf{u}_{t-1},\ldots,\textbf{u}_{t-p}$. It can be shown, that two VAR(p) models resulting from different ilr transformations are compositionally equivalent which means that the same predictions are obtained \cite{Kynclova:2015}. 
...
Assuming $n$ observations are used for the model, equation \ref{eq:VAR model} can be written in matrix form as

\begin{gather*}
\textbf{U} = \textbf{ZB} + \textbf{E}, \\
\textbf{U} = (\textbf{u}_1,\ldots,\textbf{u}_n)^T \in \mathbb{R}^{n \times (D-1)}, \\
\textbf{Z} \in \mathbb{R}^{n \times \left[(D-1)p+1\right]} \text{ with } \textbf{Z}_t = \left(1,\textbf{u}_{t-1}^T,\ldots,\textbf{u}_{t-p}^T\right)^T, \\ %von \cite{Kynclova:2015}
