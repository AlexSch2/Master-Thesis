...
Since the $ilr$ transformation is defined as a one-to-one mapping, we can transform the data back in the simplex. The inverse transformation is given by 
...
\end{gather}
%
In \textcite{Pawlowsky:2013} it is shown that $h_s=((t_s(\bm{x}),\ClOp(\bm{x}))^T)$ with $t_s(\bm{x}) = \sum_{i=1}^D x_i$ is a one-to-one function, but not compatible with $\oplus_+,\odot_+$ and $\oplus_T,\odot_T$. However, as $h_s$ is a one-to-one function between $\mathbbm{R}_+^D$ and $\Tsp$, there exists a Euclidean structure in $\mathbbm{R}_+^D$ that is isometric to the one in $\Tsp$. The vector space operations can be defined as
...
First, the Additive Replacement Strategy, which was first introduced by Aitchison in \textcite{Aitchison:1986}, and is given by
...
Third is the multiplicative replacement strategy, which is the main result of \textcite{Josep:2003}. The proposed replacement is 
...
\begin{equation}
%\hat{\bm{x}}_{non}^{(t)} = \bm{x}_{obs}^T\hat{\bm{\beta}}^{(t)} -\hat{\sigma}^{(t)}\hat{\lambda}^{(t)},
\hat{\bm{x}}_{non_j}^{(t)} = \hat{\beta}_{1j} + \sum_{i=1}^{r} x_{obs_{i}}\hat{\beta}_{(i+1),j} - \hat{\sigma}_{j}\hat{\lambda}_{j} \hspace{0.5cm} \text{for } j=1,\ldots,m,
\label{eq:E-step}
\end{equation}
%
...
%$\hat{\bm{B}} = (\hat{\bm{\beta}}_1,\ldots,\hat{\bm{\beta}}_m) \in \mathbbm{R}^{(r+1) \times m}$
