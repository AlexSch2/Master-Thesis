...
Autoregressive models of order p (AR(p))  are one of the most simple time series models, which makes them very popular. For a stationary process $\left\{x_t\right\}\ $, $\bm{a}=(a_1,\ldots,a_p)^T \in \mathbbm{R}^p$ and a white noise process $\left\{\epsilon_{t}\right\} \sim WN(\sigma^2)$, called innovations, they are defined as 
...
\end{equation}
%
As with the ARCH and GARCH models, we no longer assume that the data is constrained to integer values. The multivariate version, also called vectorised autoregressive model (VAR(p)), is defined as 

\begin{equation}
\bm{x}_{t} = \bm{a_1 x_{t-1}}+ \ldots + \bm{a_p x_{t-p}} + \bm{\epsilon}_{t},
\label{eq: Var Model}
\end{equation}
%
where $\bm{a}_j \in \mathbbm{R}^{k \times k}$ are the coefficient matrices, $\left\{\bm{\epsilon}_t \right\}\sim WN(\bm{\Sigma})$ is a multivariate white noise process with covariance matrix $\bm{\Sigma}$ and $\bm{x}_t = (x_{1t},\ldots,x_{Kt})^T$ is a K-dimensional time series.% with $\Sigma \in \mathbbm{R}^{k \times k}$. 
...
Like parameter estimation, forecasting is also simple in the AR model. We use the mean squared error as an error measure, only consider affine forecasts, assume w.l.o.g. that $\left\{\bm{x}_t\right\}$ is stationary and has expectation 0 and use $m\geq p,m \in \mathbbm{N}$ past values. So, our forecast has the form 

\begin{equation}
\hat{\bm{x}}_{t+h} = \bm{c}_1\bm{x}_{t} + \ldots + \bm{c}_m\bm{x}_{t-m+1},
...
\begin{equation}
\hat{\bm{x}}_{t+1} = \bm{a}_1\bm{x}_{t} + \ldots + \bm{a}_p\bm{x}_{t-p+1}.
\label{eq:AR 1step Forecasting}
\end{equation}
%
For $h>1$, one simply continues recursively, using $\hat{\bm{x}}_{t+1}$, $\hat{\bm{x}}_{t+2},\ldots$ \textcite{Scherrer:2021}. Since we normally do not know the exact coefficients $\bm{a}_i$, their estimations $\hat{\bm{a}}_i$ are plugged into Equation (\ref{eq:AR 1step Forecasting}). 

\subsection{Testing for AR Models}
\label{sec: Testing for ar models}

To test whether or not a time series follows an AR(p) process, the estimates of the white noise process $\left\{\hat{\epsilon}_t\right\}$ can be used. These estimates should follow a white noise process and hence should show no signs of serial correlation. Popular tests are the Portmanteau and the Breusch-Godfrey Test \textcite{Scherrer:2021}. 

The Portmanteau Test tests the null hypothesis $H_0: \mathbbm{E}[\bm{\epsilon}_t \bm{\epsilon}_{t-m}^T]=0$, i.e. if the estimated innovations are uncorrelated. Under the assumption that $\left\{\bm{x}_t\right\}$ is an AR(p) process and an AR(p) model has been fit, the used test statistic converges against a chi-squared distribution \textcite{Scherrer:2021}. 
...
\end{equation}
%
are zero, i.e. if process $\left\{\bm{\epsilon}_t\right\}$ follows an AR(h) structure or not. Under the null hypothesis the test statistic follows a chi-squared distribution again \textcite{Scherrer:2021}. 
