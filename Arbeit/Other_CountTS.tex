...
where $\epsilon_{kt} \sim WN(\sigma_k^2)$ is a white noise process with variance $\sigma_k^2 \in \mathbb{R}_+$. It can be shown easily, that the optimal 1-step ahead forecast with regards to the mean squared error (MSE) is given by
...
with $0<\omega_k<1$, $\beta_0>0$, $\beta_i\geq 0$, $\alpha_j \geq 0$ for $i=1,\ldots,p$, $j=1,\ldots,q$, $p\geq 1$, $q\geq 0$ \cite{Zhu:2012}. If $\omega_k =0$ then we get the normal INGARCH(p,q) model discussed above. It can be shown that the conditional mean and variance are given by
...
Autoregressive models of order p (AR(p))  are one of the most simple time series models, which makes them very popular. For $\bm{a}=(a_1,\ldots,a_p)^T \in \mathbbm{R}^p$ and a white noise process $\epsilon_{kt} \sim WN(\sigma_k^2)$, called innovations, they are defined as 
...
where $\bm{a}_j \in \mathbbm{R}^{k \times k}$, $\bm{\epsilon}_t \sim WN(\Sigma)$ and $\bm{Y}_t = (Y_{1t},\ldots,Y_{Kt})^T$.% with $\Sigma \in \mathbbm{R}^{k \times k}$. 
...
Like parameter estimation, forecasting is also simple in the AR model. Using the mean squared error as a measure, only considering affine forecasts and using $m\geq p,m \in \mathbbm{N}$ past values, hence 
...
we get that the optimal one-step ahead prediction is simply \cite{Scherrer:2021}
...
For $h>1$, one simply continues recursively, using $\hat{\bm{Y}}_{t+1}$, $\hat{\bm{Y}}_{t+2},\ldots$ \cite{Scherrer:2021}.
