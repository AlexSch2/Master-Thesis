...
Next we will investigate the INGARCH specific options. We use a INGARCH(1,1) with a Poisson distribution, extending windows, no external factors, no zero handling, full history $h=1$ and initial window length $w=0.5$ as the standard model. We will always vary one setting and leave the others unchanged. 
...
Lastly we will compare different CoDA Specifications as mentioned in \ref{sec: Coda Specifications}. Like above we choose one standard model for comparison and always only change one setting. For CoDA our standard model uses extending windows, the full history $h=1$, an initial window length of $w=0.5$, add 0.5 to all values, $\Tsp$-spaces with the logarithmic sum and the one-vs-all method. 
