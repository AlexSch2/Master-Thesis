---
title: "one_vs_all_analysis_script"
output: pdf_document
date: '2023-01-10'
---
...
data_fit<-dplyr::select(data_fit,y1,yother,x1,xother)
...
```{r}
model<-vgam(cbind(y1,yother)~s(x1)+s(xother),data=data_fit,family = poissonff(link = "identitylink",imethod=1),
...
prediction_results<-bind_rows(lapply(c((fit_length+1):(raw_length-1)),function(index){
  ...
  data_test<-data_prep[index,c("x1","xother")]
  
  predicted_values<-as.vector(predict(model,data_test))
  
  true_values<-as.vector(unlist(data_prep[index,c("y1","yother")]))
  
  return(data.frame(group=factor(rep(c("1","other"),2)),
                    type=rep(c("predicted_value","true_value"),each=2),
                    prediction_values=c(predicted_values,true_values)))
  
}))
prediction_results$week_date<-rep(data_raw$week_date[(fit_length+1):(raw_length-1)],each=4)
```
