...
PredictionStep <- 1
frame <- unique(coda_result$result$window_baseLength)
...
if(!identical(ids,unique(ingarch_result$result$id)) || 
   frame != unique(ingarch_result$result$window_baseLength) ||
...
coda_model_error <- Model.Error(coda_result,Fnct = "Mse") %>% Model.ErrorOverall(Fnct = "sum")
ingarch_model_error <- Model.Error(ingarch_result,Fnct = "Mse") %>% Model.ErrorOverall(Fnct = "sum")
...
```{r}
my_color <- setNames(c("black", "white"),
                     c("1,2","3,4"))

ps<-ggplot(model_error_split,aes(x=id,y=error))+
  geom_point(size=5,shape=21,aes(fill=model,colour=group),stroke=2)+
  scale_y_continuous(limits=c(0,3))+
  scale_colour_manual("Legend", values = c(my_color))+
  theme(text = element_text(size = text_size))+
  ggtitle(paste("Fridge_error_measures",sep=" "),subtitle = paste("Base window length:",frame,sep=" "))
...
```{r}
my_color <- setNames(c("yellow", "green"),
                     c("1,2","3,4"))
...
if(save_plots){
ggsave(filename = here("Plots",paste("rmse_cum",FID,"win_lgth",frame,".png",sep="")),plot=p8,height = 15,width = 20)
