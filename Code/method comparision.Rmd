...
plot_data_coda<-coda_result$results%>%dplyr::filter(category %in% c(1,2,3,4))%>%dplyr::select(!c(naive_predicted_value,prediction_error_naive))
...
p3 <- ggplot(plot_data,aes(x=prediction_date,y=prediction_error_normed,col=model))+
  ...
  ggtitle(paste("Normed prediction error over time ",sep=" "),subtitle = paste("Fridge ID:",id,sep=" "))+
  scale_color_manual("Legend", values = c(my_color))
...
```{r}
plot_coda_lag <- coda.get.lag(coda_result)
p5 <- ggplot(plot_coda_lag,aes(x=window,y=lag))+
  geom_point(size=1.5)+
  geom_line()+
  ggtitle(paste("Coda model lag per window",sep=" "),subtitle = paste("Fridge ID:",id,sep=" "))+
  theme(text = element_text(size = text_size))

if(one_vs_all)p5+facet_wrap(vars(category),nrow=length(unique((plot_coda_lag$category))),scales = "free")
```
