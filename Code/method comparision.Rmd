...
ingarch_result <-
  ...
  if(save_results){
  save(ingarch_result,file=here("Code/results",paste("ingarch_result_ids",paste(ids_save,collapse="-"),"_ext","_",F,
                                             "_win_lgth",gsub("[.]","",frame),"_history_lght",gsub("[.]","",HistoryLength),
                                             "_pobs",1,"_pmeansu",1,".RData",sep="")))
...
if(save_plots){
  ggsave(filename = here("Plots",paste("Both_ErrorMeasure_combined",ids_save,"_histlgth",HistoryLength,"_win_lgth",frame,".png",sep="")),plot=psbs,height = 15,width = 20)
}
```
...
plot_quant <- ggplot(IngarchError_Sorted,aes(x=index,y=error,colour=model))+
  geom_point(size=3)+
  scale_y_continuous(limits=c(0,10))+
  geom_point(data = CodaError_Sorted,aes(x=index,y=error,colour=model),size=3)+
...
plot_quant_split <- ggplot(Error_Sorted_split ,aes(x=index,y=error,colour=model))+
...
ps<-ggplot(model_error_split,aes(x=id,y=error))+
  ...
  scale_colour_manual("Legend", values = c(my_color_methods),aesthetics = "colour")+
  theme(text = element_text(size = text_size),axis.text.x = element_text(angle = 45, vjust = 0.7, hjust=1,size=10))+
  ggtitle(paste("Error measures sorted, History:",HistoryLength,sep=" "),subtitle = paste("Window length:",frame,sep=" "))

if(save_plots){
  ggsave(filename = here("Plots",paste("Both_ErrorMeasure_PointPlot",ids_save,"_histlgth",HistoryLength,"win_lgth",frame,".png",sep="")),plot=ps,height = 15,width = 20)
}
```
...
p<-ggplot(plot_data_ingarch,aes(x=date,y=valuePredict),col="blue")+
  ...
  theme(text = element_text(size = text_size))+
  ggtitle(paste("Ingarch Timeseries",sep=" "),subtitle = paste("Fridge ID:",FID,",base window length:",frame,sep=" "))
