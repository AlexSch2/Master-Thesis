...
if(save_results){
  save(coda_result,file=here("Code/results",paste("coda_result_ids",paste(ids_save,collapse="-"),"win_lgth",gsub("[.]","",frame),
                                                  "_history_lght",gsub("[.]","",HistoryLength),".RData",sep="")))
}
...
ingarch_result <-
  ...
  save(ingarch_result,file=here("Code/results",paste("ingarch_result_ids",paste(ids_save,collapse="-"),
                                             "_win_lgth",gsub("[.]","",frame),"_history_lght",gsub("[.]","",HistoryLength),".RData",sep="")))
...
```{r}
Coda1 <- Object.Load("~/Documents/Uni/Masterarbeit/Simulation/Code/Master-Thesis/Simulation/Code/results/coda_result_ids_all_win_lgth05_history_lght1.RData")
Coda05 <- Object.Load("~/Documents/Uni/Masterarbeit/Simulation/Code/Master-Thesis/Simulation/Code/results/coda_result_ids_all_win_lgth05_history_lght05.RData")

Ingarch1 <- Object.Load("~/Documents/Uni/Masterarbeit/Simulation/Code/Master-Thesis/Simulation/Code/results/ingarch_result_ids_all__win_lgth05_history_lght1.RData")
Ingarch05 <- Object.Load("~/Documents/Uni/Masterarbeit/Simulation/Code/Master-Thesis/Simulation/Code/results/ingarch_result_ids_all__win_lgth05_history_lght05.RData")

Coda_Combined <- rbind(Coda1$result,Coda05$result)
Ingarch_Combined <- rbind(Ingarch1$result,Ingarch05$result)

Plot.ErrorMeasureCombined(CodaCombined = Coda_Combined,IngarchCombined = Ingarch_Combined,Values=c("0.5","1"),Split=F,Variation = "history")
...
ingarch_model_error_split <- Model.Error(ingarch_result,Fnct = "Mse",Category =as.numeric(unique(ingarch_result$result$category))) %>% Model.ErrorOverall(Fnct = "sum",SplitByGroup = T,Groups = list(6,7,9,10,13))
...
```{r}
psb<-ggplot(ingarch_model_error,aes(x=model,y=error))+
  geom_boxplot()+
...
if(save_plots){
  ggsave(filename = here("Plots",paste("Both_ErrorMeasure_split_zoomed",ids_save,"_histlgth",HistoryLength,"win_lgth",frame,".png",sep="")),plot=psbs_zoomed,height = 15,width = 20)
}
...
IngarchError_Sorted_split <- model_error_split %>% filter(model=="ingarch")%>%group_by(group)%>%arrange(.,error)
IngarchQuantiles_split  <- quantile(IngarchError_Sorted_split $error,na.rm = T)

IngarchError_Sorted_split$index <- NA
IngarchError_Sorted_split[IngarchError_Sorted_split$group=="1,2",]$index <- seq(1:round(dim(IngarchError_Sorted_split )[1]/2))
IngarchError_Sorted_split[IngarchError_Sorted_split$group=="3,4",]$index <- seq(1:round(dim(IngarchError_Sorted_split )[1]/2))
...
CodaError_Sorted_split  <- model_error_split  %>% filter(model=="coda")%>%arrange(.,error)
CodaQuantiles_split  <- quantile(CodaError_Sorted_split $error,na.rm = T)

CodaError_Sorted_split$index <- NA
CodaError_Sorted_split[CodaError_Sorted_split$group=="1,2",]$index <- seq(1:round(dim(CodaError_Sorted_split )[1]/2))
CodaError_Sorted_split[CodaError_Sorted_split$group=="3,4",]$index <- seq(1:round(dim(CodaError_Sorted_split )[1]/2))
CodaError_Sorted_split <- full_join(CodaError_Sorted_split,TimeSeries_Length,bye ="id")

Error_Sorted_split <- rbind(CodaError_Sorted_split,IngarchError_Sorted_split)
...
if(save_plots){
  ggsave(filename = here("Plots",paste("Quantile_Plot_Split",ids_save,"_histlgth",HistoryLength,"win_lgth",frame,".png",sep="")),plot=plot_quant_split,height = 15,width = 20)
}
```
