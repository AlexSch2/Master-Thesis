...
Count_Result <- rbind(ingarch_result$result,
                 inarzim_result$result,
                 zim_result$result)

Common_Names <- intersect(names(ingarch_result$result),names(coda_result$result))
CodaResult_Short <- coda_result$result %>% select(all_of(Common_Names))
...
Plot.MethodComparision(AllResult,Category = c(1,2,3,4),ZIM=F)
