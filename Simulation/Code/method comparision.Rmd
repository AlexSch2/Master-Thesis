...
ingarch_model_error <- Model.Error(ingarch_result,Fnct = "Mse",Category = unique(ingarch_result$result$category)) %>% Model.ErrorOverall(Fnct = "sum",SplitByGroup = F)
...
ingarch_model_error_split <- Model.Error(ingarch_result,Fnct = "Mse",Category = unique(ingarch_result$result$category)) %>% Model.ErrorOverall(Fnct = "sum",SplitByGroup = T,Groups = list(6,7,9,10,13))
...
data_raw <- weekly_category_data %>%
          filter(fridge_id == FID&
                   main_category_id %in% unique(ingarch_result$result$main_category)) %>%
          dplyr::select(week_date, main_category_id, sub_category_id, sold) %>%
          Data.Preparation(Category = unique(ingarch_result$result$category),TakeSubCategory = T) %>% 
